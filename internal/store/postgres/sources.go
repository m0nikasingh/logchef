...
func (db *DB) CreateSource(ctx context.Context, source *models.Source) error {
	...
	if err != nil {
		...
		// handleUniqueConstraintError already returns a wrapped store.ErrUniqueConstraint
		// for every 23505 (including composite (database, table_name) collisions), so
		// no second fallback branch is needed.
		if isUniqueConstraintPostgresError(err) {
			return handleUniqueConstraintError(err, "sources", "name", source.Name)
		}
