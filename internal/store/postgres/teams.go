...
import (
	...
	sqlc "github.com/mr-karan/logchef/internal/store/postgres/sqlc"
	sqlitesqlc "github.com/mr-karan/logchef/internal/store/sqlite/sqlc"
...
func (db *DB) CreateTeam(ctx context.Context, team *models.Team) error {
	...
	if err != nil {
		...
		if errors.As(err, &pgErr) && pgErr.Code == "23505" {
			...
			return fmt.Errorf("creating team: %w", store.ErrUniqueConstraint)
		}
...
func (db *DB) UpdateTeam(ctx context.Context, team *models.Team) error {
	...
	if err != nil {
		...
		if errors.As(err, &pgErr) && pgErr.Code == "23505" {
			...
			return fmt.Errorf("updating team: %w", store.ErrUniqueConstraint)
		}
...
func (db *DB) AddTeamMember(ctx context.Context, teamID models.TeamID, userID models.UserID, role models.TeamRole) error {
	...
	if err != nil {
		...
		var pgErr *pgconn.PgError
		if errors.As(err, &pgErr) && pgErr.Code == "23505" {
			if pgErr.ConstraintName == "team_members_pkey" {
				// Not a fatal error; treat re-add as no-op.
				db.log.Warn("attempted to add existing team member", "team_id", teamID, "user_id", userID)
				return nil
			}
			return fmt.Errorf("adding team member: %w", store.ErrUniqueConstraint)
		}
...
func (db *DB) AddTeamSource(ctx context.Context, teamID models.TeamID, sourceID models.SourceID) error {
	...
	if err != nil {
		...
		var pgErr *pgconn.PgError
		if errors.As(err, &pgErr) && pgErr.Code == "23505" {
			if pgErr.ConstraintName == "team_sources_pkey" {
				db.log.Warn("attempted to add existing team-source association", "team_id", teamID, "source_id", sourceID)
				return nil
			}
			return fmt.Errorf("adding team source: %w", store.ErrUniqueConstraint)
		}
...
// The Store interface still references sqlite-internal sqlc.ListTeamsForUserRow; until that interface leak
// is resolved (see T22 concern notes), this method converts field-by-field at the boundary. The postgres and
// sqlite shapes are structurally identical.
func (db *DB) ListTeamsForUser(ctx context.Context, userID models.UserID) ([]sqlitesqlc.ListTeamsForUserRow, error) {
	...
	out := make([]sqlitesqlc.ListTeamsForUserRow, 0, len(teamRows))
	for _, r := range teamRows {
		out = append(out, sqlitesqlc.ListTeamsForUserRow{
			ID:          r.ID,
			Name:        r.Name,
